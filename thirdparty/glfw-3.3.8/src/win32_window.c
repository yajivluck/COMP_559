                                    ...
                                    int windowWidth, int windowHeight)
{
    ...
    for (i = 0;  i < count;  i++)
    {
        const int currDiff = abs(images[i].windowWidth * images[i].windowHeight -
                        ...
                        int xhot, int yhot, GLFWbool icon)
{
    ...
    bi.bV5Width       = image->windowWidth;
    bi.bV5Height      = -image->windowHeight;
    bi.bV5Planes      = 1;
    bi.bV5BitCount    = 32;
    ...
    mask = CreateBitmap(image->windowWidth, image->windowHeight, 1, 1, NULL);
    if (!mask)
    {
        _glfwInputErrorWin32(GLFW_PLATFORM_ERROR,
    ...
    for (i = 0;  i < image->windowWidth * image->windowHeight;  i++)
    {
        target[0] = source[2];
        target[1] = source[1];
        target[2] = source[0];
                                   ...
                                   WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_SIZE:
        {
            const int windowWidth = LOWORD(lParam);
            const int windowHeight = HIWORD(lParam);
            ...
            if (windowWidth != window->win32.windowWidth || windowHeight != window->win32.windowHeight)
            {
                window->win32.windowWidth = windowWidth;
                window->win32.windowHeight = windowHeight;

                _glfwInputFramebufferSize(window, windowWidth, windowHeight);
                _glfwInputWindowSize(window, windowWidth, windowHeight);
            }
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    else
    {
        ...
        getFullWindowSize(style, exStyle,
                          wndconfig->windowWidth, wndconfig->windowHeight,
    ...
    if (!window->monitor)
    {
        RECT rect = { 0, 0, wndconfig->windowWidth, wndconfig->windowHeight };
    ...
    _glfwPlatformGetWindowSize(window, &window->win32.windowWidth, &window->win32.windowHeight);
...
void _glfwPlatformGetWindowSize(_GLFWwindow* window, int* windowWidth, int* windowHeight)
{
    RECT area;
    GetClientRect(window->win32.handle, &area);

    if (windowWidth)
        *windowWidth = area.right;
    if (windowHeight)
        *windowHeight = area.bottom;
}

void _glfwPlatformSetWindowSize(_GLFWwindow* window, int windowWidth, int windowHeight)
{
    if (window->monitor)
    {
        if (window->monitor->window == window)
...
void _glfwPlatformGetFramebufferSize(_GLFWwindow* window, int* windowWidth, int* windowHeight)
{
    _glfwPlatformGetWindowSize(window, windowWidth, windowHeight);
}
                                   ...
                                   int refreshRate)
{
    if (window->monitor == monitor)
    {
        ...
        {
            RECT rect = { xpos, ypos, xpos + windowWidth, ypos + windowHeight };
...
void _glfwPlatformPollEvents(void)
{
    ...
    if (window)
    {
        ...
        if (window->win32.lastCursorPosX != windowWidth / 2 ||
            window->win32.lastCursorPosY != windowHeight / 2)
        {
            _glfwPlatformSetCursorPos(window, windowWidth / 2, windowHeight / 2);
        }
    }
}
