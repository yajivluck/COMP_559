...
bool mouseRightDown = false; //New bool to detect a right mouse click
bool right_button_pressed = false; //New bool detect right button kept pressed
bool Attraction = true;	//bool to display force attraction or repulsion being displayed
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if (key == GLFW_KEY_ESCAPE) {
		...
		showKeyBindings = (showKeyBindings+1)%4;
	} else if (key == GLFW_KEY_T) {					//Naive Gravity
		particleSystem.useGravity = false;
		particleSystem.BarnesHut = false;
		particleSystem.Naive_Gravity = true;
		particleSystem.ccd.useRepulsion = true;

	}else if (key == GLFW_KEY_Q) {					
		particleSystem.useGravity = false;
		particleSystem.BarnesHut = true;
		particleSystem.Naive_Gravity = false;
		particleSystem.ccd.useRepulsion = false;
		//Adding custom toggle to work with "outer space" gravity
	...
	} else if (key == GLFW_KEY_G) {	//Set to Atmospheric Gravity

		particleSystem.useGravity = true;
		particleSystem.BarnesHut = false;
		particleSystem.Naive_Gravity = false;

		//particleSystem.gravity *= scale;  //No longer scale original gravity
...
void mouse_callback(GLFWwindow* window, int button, int action, int mods) {
	if (button == GLFW_MOUSE_BUTTON_LEFT) {
		...
		if (!run) {
		...
		} else {											//While Running, implement mouse gravity field. Mouse field opposite to current forces
			if (pressed) {
				xdown = xcurrent;
				ydown = ycurrent;
				...
				//if (p1 != NULL && d1 < grabThresh) {
					...
					//particleSystem.mouseSpring.p1 = p1;		//No mouse Spring to be used from now on
					//particleSystem.useMouseSpring = true;
				//}

				
				particleSystem.GravityField.mass = (Attraction) ? -20000 : 20000; //Opposite Field
				particleSystem.useGravityField = true;
				particleSystem.GravityField.p.x = xcurrent;	//Field follows mouse similarly to mouseSpring
				particleSystem.GravityField.p.y = ycurrent;
				particleSystem.GravityField.pinned = true; //Pin Point being added

				
				particleSystem.particles.push_back(&particleSystem.GravityField);
			...
			if (released) {
				...
				particleSystem.GravityField.mass = 0 ; //Erase Gravity Field
				particleSystem.GravityField.p.x = NULL;	//Field follows mouse similarly to mouseSpring
				particleSystem.GravityField.p.y = NULL;
				particleSystem.useGravityField = false;
				particleSystem.remove(&particleSystem.GravityField);
	...
	if (button == GLFW_MOUSE_BUTTON_RIGHT){
		...
		if (action == GLFW_PRESS && !right_button_pressed)
		{
			// Right mouse button was pressed for the first time


			Attraction = !Attraction; //Toggle Attraction boolean	
			particleSystem.toggle_G(Attraction); //Invert gravitational forces opposite

			//For all particles in the system, if there is a right click
			//for (Particle* particle : particleSystem.particles) {
				//if (particle->pinned) {
					//particle->mass = -particle->mass;	//Invert mass of pinned particles on a right click
				//}
			//}
...
static void render() {
	...
	if (showKeyBindings == 0) {
		...
		ss << "[g] Atmospheric gravity = " << particleSystem.useGravity << "\n";
		ss << "[q] Barnes-Hut gravity = " << particleSystem.BarnesHut << "\n";
		ss << "[t] Naive Gravity = " << particleSystem.Naive_Gravity << "\n";
		ss << "[Right Click] Type of Force = " << (Attraction ? "Attraction" : "Repulsion") << "\n";
...
int main(int argc, char** argv) {
	...
	window = glfwCreateWindow(1280, 720, "COMP 559 W23 - A2 CCD - YOUR NAME", NULL, NULL);
	if (!window) {
		glfwTerminate();
		return -1;
	}
