 ...
 */
	int createSystem(int which) {
		...
		if (which == ID++) {
			...
			//system->name = std::to_string(which) + " - Barnes-Hut 100 Particles";
			//CreateBarnesHut(100);
		}else if (which == ID++) {
			//system->name = std::to_string(which) + " - Pendulum Pair";
			//createPendulumPair();
			system->name = std::to_string(which) + " - Naive Gravity 100 particles Max distance of 50";
			CreateNaiveGravity(100,50);
		}else if (which == ID++) {
			//system->name = std::to_string(which) + " - Pendulum Pair";
			//createPendulumPair();
			system->name = std::to_string(which) + " - Naive Gravity 100 particles Max distance of 100";
			CreateNaiveGravity(100, 100);
		}else if (which == ID++) {
			//system->name = std::to_string(which) + " - Pendulum Pair";
			//createPendulumPair();
			system->name = std::to_string(which) + " - Naive Gravity 100 particles Max distance of 500";
			CreateNaiveGravity(100, 500);
		}else if (which == ID++) {
			//system->name = std::to_string(which) + " - Pendulum Pair";
			//createPendulumPair();
			system->name = std::to_string(which) + " - Naive Gravity 100 particles No Max Distance";
			CreateNaiveGravity(100, INFINITY);
		}  else if (which == ID++) {
			//string scene = "pendulumPair.xml";
			//system->name = std::to_string(which) + " - " + scene;
			//parser.load(system, RES_DIR + scene);
			system->name = std::to_string(which) + " - Naive Gravity 500 particles Max distance of 50";
			CreateNaiveGravity(500,50);
		} else if (which == ID++) {
			system->name = std::to_string(which) + " - Naive Gravity 1000 particles Max distance of 50";
			CreateNaiveGravity(1000,50);
			//system->name = std::to_string(which) + " - Bullet And Layers";
			//createBulletAndLayers();
		}else if (which == ID++) {												//Added BarnesHut system as first system to appear from the list
			system->name = std::to_string(which) + " - Barnes-Hut 100 Particles";
			CreateBarnesHut(100);							//Added BarnesHut system as first system to appear from the list
		}else if (which == ID++) {												//Added BarnesHut system as first system to appear from the list
			system->name = std::to_string(which) + " - Barnes-Hut 500 Particles";
			CreateBarnesHut(500);							//Added BarnesHut system as first system to appear from the list			
		}else if (which == ID++) {
			system->name = std::to_string(which) + " - Barnes-Hut 1000 Particles";
			CreateBarnesHut(1000);
			//system->name = std::to_string(which) + " - Simple";
			//createSimple();
			//system->name = std::to_string(which) + " - Test";
			//createTest();
		}
...
/**
 * Creates a pinned box that matches the current display window
 */
	void createBox() {
		...
		//double h = mode->windowHeight;
		//double w = mode->windowWidth;		//Retrieving full screen display.
											//SIMULATION TO BE DONE ON FULL SCREEN AS TO HAVE MORE SPACE AND BETTER VISUALS


		double h = system->height;		//These are for small screen display
		double w = system->width;

		//Particle* p1 = system->createParticle(5, 5, 0, 0); p1->pinned = true;							//PREVIOUS COORDINATES FOR BOX IN "SMALL WINDOW" SIZE
		...
		Particle* p1 = system->createParticle(0, 0, 0, 0); p1->pinned = true;	//BOX WITH BOUNDS OUTSIDE OF VISION
		Particle* p2 = system->createParticle(0, h, 0, 0); p2->pinned = true;
		Particle* p3 = system->createParticle(w, h, 0, 0); p3->pinned = true;
		Particle* p4 = system->createParticle(w , 0, 0, 0); p4->pinned = true;


		system->createSpring(p1, p2);
		system->createSpring(p2, p3);
		system->createSpring(p3, p4);
		system->createSpring(p4, p1);  //Closed top of Box as to fully bound system
	}


	//Function that instantiate a three body orbiting structure centered at x_center, y_center
	//MASS_MIDDLE, MASS_ORBIT1, AND MASS_ORBIT2 ARE FRACTIONS (BETWEEN 0 AND 1) RELATIVE TO THE MAXIMUM MASS ALLOWED IN THE SYSTEM
	void CreateOrbit(int x_center, int y_center, double mass_middle, double mass_orbit1, double mass_orbit2) {

		Particle* p1 = new Particle(x_center, y_center, 0, 0);		//MIDDLE		//Massive (pinned) celestial body in the middle of the screen
		p1->pinned = true; //Only pin p1 as a "stationary massive celestial body
		int MAX_MASS = p1->get_max_mass();
		p1->mass = MAX_MASS * mass_middle;  //MIDDLE MASS HAS FRACTIONAL MASS RELATIVE TO MAXIMUM ALLOWED MASS (10 000)
		p1->set_Color();


		Particle* p2 = new Particle(x_center - 30, y_center + 10, 22, 10);		//RIGHT
		p2->mass = mass_orbit1 * MAX_MASS; //FRACTION TO MIDDLE BODY
		p2->set_Color();


		Particle* p3 = new Particle(x_center + 30, y_center - 10, -22, -10);		//RIGHT
		p3->mass = mass_orbit2 * MAX_MASS; //FRACTION TO MIDDLE BODY
		p3->set_Color();
...
private:
	...
	/**
	* Adding my own system to test my implementation of the final project. This system will be used to test the Barnes-Hut approximation
	* to estimate gravitational forces between multiple particles
	*/

	void CreateBarnesHut(int num_particles) {

		system->useGravity = false;
		system->Naive_Gravity = false;	//Not Naive Gravity
		system->BarnesHut = true;	//Set Gravity Type to Barnes Hut
		system->viscousDamping = 0.15; //Set damping to prevent system from "exploding" when particles are too close to each other
		...
		//Make N random particles, 500 is fast with Barnes Hut, 1000 is probably limit, on my computer, 2000 starts to not be smooth
		for (int particle = 0; particle < num_particles; particle++) {
			Particle* p = new Particle(

				randRange(0, w),     // random x position within full screen
				randRange(0, h),    // random y position  within full screen
				randRange(-5, 5),        // random x velocity
				randRange(-5, 5)         // random y velocity
			);

			//p->mass = 1;
			p->mass = randRange(500, 10000);  // random mass between 2000 and max mass
			p->set_Color();
	...
	//Test System implementing Naive Gravity
	void CreateNaiveGravity(int num_particles, double dist) {

		system->useGravity = false;
		system->Naive_Gravity = true;	//Use Naive Gravity
		system->BarnesHut = false;	//Set Gravity Type to Barnes Hut
		system->viscousDamping = 0.15; //Set damping to prevent system from "exploding" when particles are too close to each other

		//system->max_dist = sqrt(system->width * system->width + system->height * system->height); 


		system->max_dist = dist;

		//system->ccd.useRepulsion = true; //Apply CCD 

		//Radius of computable gravity amongst particles
		...
		for (int particle = 0; particle < num_particles; particle++) {
			Particle* p = new Particle(
				...
				//randRange(2*w/5, 3*w/5),     // random x position within fifth of screen
				//randRange(2*w/5, 3*h/5),    // random y position  within fifth of screen

				randRange(0, w),     // random x position within fifth of screen
				randRange(0, h),    // random y position  within fifth of screen
	...
	//Test system for whatever scenario I want to analyse
	void createTest() {
		...
		Particle* p1 = new Particle(100, 96, 10, 0);
		Particle* p2 = new Particle(150, 100, 0, 0);

		particles->push_back(p1);
		particles->push_back(p2);

		p1->mass = 1;  // random mass between 2000 and max mass
		p2->mass = 10;  // random mass between 2000 and max mass
